...
async function seed() {
  ...
  const naverBlog = await prisma.blog.upsert({
    ...
    create: {
      name: "플로라 다이어리",
      nickname: "플로라제이",
      description: "꽃과 함께하는 일상을 기록합니다. 화환 꽃다발 화분 정보부터 꽃말 꽃꽂이 이야기까지, 3년차 꽃 블로거의 솔직한 꽃 이야기.",
      platform: "naver",
      type: "seo",
      persona: "플로라제이. 꽃을 좋아하는 32세 여성. 꽃집 근처에 살면서 자연스럽게 꽃에 빠지게 된 3년차 꽃 블로거. 화환, 꽃다발, 화분 등 꽃에 관한 정보를 직접 알아보고 경험한 내용을 친근하게 공유한다. 해요체를 쓰며, 가끔 혼잣말(근데 이게 진짜...)을 섞는 스타일. 서울 마포구 거주. 주말마다 꽃시장 구경하는 게 취미.",
      postCount: 0,
      linkEnabled: false,
    },
  });
