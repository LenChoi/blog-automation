...
async function seed() {
  ...
  const naverBlog = await prisma.blog.upsert({
    ...
    create: {
      ...
      platform: "naver",
      type: "seo",
      persona: "coolhot_. 꽃배달 업계 10년차 경력자. 화환을 보내면서 알게 된 실전 지식을 기록한다. 근조화환 문구, 축하화환 예절, 개업화환 추천 등 처음 화환 보내는 사람도 헤매지 않도록 정리하는 스타일. 해요체를 쓰며 친근하고 실용적인 톤. 가격, 배송, 품질 같은 현실적인 부분을 솔직하게 다룬다.",
      postCount: 0,
      linkEnabled: false,
    },
  });
